...
import { useAuth } from "@/contexts/AuthContext"; // Importando o contexto real
...
export function Header() {
  ...
  // Obtendo dados reais do contexto
  ...
  const departments = [
    ...
    {
      name: "Informática & Acessórios",
      icon: Monitor,
      slug: "informatica-e-acessorios",
    },
    {
      name: "Saúde, Nutrição & Bem-Estar",
      icon: Heart,
      slug: "saude-nutricao-e-bem-estar",
    },
    {
    ...
    {
      name: "Áudio, Vídeo & Mobile",
      icon: Smartphone,
      slug: "audio-video-e-mobile",
    },
  ];
        ...
        <header className="bg-[#2563EB] text-white py-4 px-4 h-[76px]">
          <div className="container mx-auto flex items-center justify-between gap-8 h-full">
            ...
            <div className="flex items-center gap-4 md:gap-6">
              {/* LÓGICA DE LOGIN / LOGOUT REAL */}
              {!user ? (
                /* Botão para Visitante (Não Logado) */
                <Link
                  href="/login"
                ...
                >
                  <button className="flex items-center gap-3 text-white hover:bg-blue-600 px-3 py-2 rounded-md transition-colors">
                    <div className="bg-white/20 p-1.5 rounded-full">
                      <User className="w-5 h-5" />
                    </div>
                    <div className="text-left leading-tight text-xs">
                      {/* Nome do Usuário Dinâmico */}
                      <div className="">Olá, {user.firstName || "Cliente"}</div>
              ...
              <Link
                href="/carrinho"
                className="relative p-2 hover:bg-blue-600 rounded-full transition-colors hidden md:block"
              >
      ...
      <nav className="bg-white border-b border-gray-100 shadow-sm hidden md:block sticky top-0 z-40">
        <div className="container mx-auto flex items-center relative">
          ...
          <div className="flex items-center overflow-x-auto no-scrollbar mask-gradient">
            {departments
              .filter((d) => d.slug !== "audio-video-e-mobile")
              .map((item) => (
                <Link
                  key={item.name}
